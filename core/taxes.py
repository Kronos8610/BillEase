...
from core.models import IVA_GENERAL, Linea  # noqa: F401  (Linea se reexporta)
from core.money import CENTIMO, a_decimal, formatear, redondear  # noqa: F401
...
def desde_detalles(detalles, tipo_iva=IVA_GENERAL):
    ...
    Acepta filas que se leen por nombre de columna y también las tuplas de la
    forma antigua `(num_factura, num_linea, cantidad, precio, cod_servicio,
    descripcion)`, para no romper el código que todavía las construya a mano.
    """
    lineas = []
    for fila in detalles:
        claves = set(fila.keys()) if hasattr(fila, "keys") else set()
        if {"NumServicios", "precioPorServicio"} <= claves:
            lineas.append(Linea.desde_fila(fila, tipo_iva=tipo_iva))
        else:
            lineas.append(
                Linea(
